...
## 架构图
 ...
 1. 任何商户平台的配置信息都统一由`go-wxpay-gateway`集中管理，应用只要通过`应用名`、`appId`就可以使用这些参数
 1. 任何通知结果的回调信息都统一由`go-wxpay-gateway`接收，然后由`go-wxpay-notify`程序根据应用配置的回调参数进行分发
 1. 支付流程、支付结果通知流程
     ...
     1. `微信商户平台`返回相应的支付参数
     1. `go-wxpay-gateway`抽取支付相关的参数，生成或包装`激活微信支付`的参数，返回给`微信支付应用`
     ...
     1. 结果通知服务`go-wxpay-notify`一直侦听`结果通知日志`的变化，把结果分发给指定的`支付回调`URL
...
## 下载、编译方法
 ...
 2. 进入任一文件夹，执行命令
    ...
    ```
 3. 编译成功，会得到3个可执行程序
     1. `go-wxpay-gateway`: 微信支付网关程序，可以执行`./go-wxpay-gateway -v`显示程序信息。
     1. `go-wxpay-notify`:  支付结果通知服务程序，可以执行`./go-wxpay-notify -v`显示程序信息。
     1. `go-wxpay-getsandbox`: 获取沙箱测试的appKey工具，可以执行`./go-wxpay-getsandbox -v`显示程序信息。
...
## go-wxpay-gateway服务的接口参数
 1. 创建支付
    - 对应配置项: `create-pay`
    - 访问方法: POST
    - URI: 如配置值为`/wxpay/create-pay`，则通过`/wxpay/create-pay/<type>`访问
       - &lt;type&gt;的可取值为`JSAPI`、`H5`、`APP`、`NATIVE`
    - 请求参数
       1. type为JSAPI的参数
